...
const TOKENS_FILE = path.join(__dirname, '../../data/gmail-tokens.json')
const SCOPES = [
...
// GET /auth/gmail — show authorization page
router.get('/', (req, res) => {
...
<html>
<head>
  <title>Connect Gmail — TCF Dashboard</title>
  <style>
...
<body>
  <div class="card">
    <h1>Connect Gmail Accounts</h1>
    <p class="sub">Authorize each account one at a time. Click Allow on Google's page.</p>
    ...
    ${nextUnauthorized
      ? `<a class="btn" href="/auth/gmail/start?email=${encodeURIComponent(nextUnauthorized.email)}">
      ...
      : `<div class="btn done">✓ All accounts connected!</div>
         <p class="note">All 3 Gmail accounts are now connected to your dashboard.</p>`
    }
...
// GET /auth/gmail/start?email=xxx — start OAuth for specific account
...
// GET /auth/gmail/callback — Google redirects here after authorization
router.get('/callback', async (req, res) => {
  const { code, state, error } = req.query

  if (error) {
    return res.send(`<p style="color:red;font-family:sans-serif;padding:2rem">Error: ${error}. <a href="/auth/gmail">Try again</a></p>`)
  }
  ...
  try {
    ...
    console.log(`Gmail authorized: ${email}`)
    res.redirect('/auth/gmail')
  } catch (err) {
    console.error('Gmail auth callback error:', err.message)
    res.send(`<p style="color:red;font-family:sans-serif;padding:2rem">Error: ${err.message}. <a href="/auth/gmail">Try again</a></p>`)
  }
})

// GET /auth/gmail/status — check which accounts are connected
